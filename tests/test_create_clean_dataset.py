...
def _make_listings_df(n=5) -> pd.DataFrame:
    ...
    return pd.DataFrame({
        ...
        "host_response_rate":    [f"{100 - (i % 5) * 10}%" for i in range(n)],
        "host_acceptance_rate":  [f"{95 - (i % 5) * 10}%" for i in range(n)],
        ...
        "bathrooms_text":        (["1 bath", "2 baths", "Half-bath", "1.5 baths", "3 baths"] * n)[:n],
        "amenities":             ['["Wifi", "Kitchen"]'] * n,
        "room_type":             (["Entire home/apt", "Private room", "Shared room"] * n)[:n],
        "neighbourhood_cleansed":(["Mission", "SoMa", "Castro", "Noe Valley", "Marina"] * n)[:n],
